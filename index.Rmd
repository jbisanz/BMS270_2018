...
```{r, include=F}
knitr::opts_chunk$set(echo = TRUE, message=TRUE, warning=FALSE,  cache=FALSE, tidy=FALSE)
```
...
untar("TutorialData.tar.gz")
```

<b>If you are using windows, please download from this [link](https://github.com/jbisanz/BMS270_2018/raw/master/TutorialData.tar.gz), and decompress in the same directory as <i>YourName.MicrobiomeTutorial.R</i></b>
...
If for some reason you could not download the reads from the ENA you can download them [here](https://github.com/jbisanz/BMS270_2018/raw/master/Rawdata.tar.gz).
...
```{r}
readLines("TutorialData/RawData/2024.birth.MNIH02UP.BFF10F.V.fastq.gz", n=4) #read top 4 lines of file and print to screen
```
...
```{r}
FASTQs<-paste0("TutorialData/RawData/", list.files("TutorialData/RawData")) # generate a list of file names
...
If you were unable to filter the reads on your own computer, you can download the filtered reads [here](https://github.com/jbisanz/BMS270_2018/raw/master/FilteredData.tar.gz).
...
#error_profile<-readRDS("TutorialData/RDS/error_profile.RDS") #to load from file
```
...
#derep<-readRDS("TutorialData/RDS/derep.RDS") #to load from file
```
...
#denoised<-readRDS("TutorialData/RDS/denoised.RDS") #to load from file
```
...
#sv.table<-readRDS("TutorialData/RDS/sv.table.RDS") # to load from file
```
...
#sv.table.nochim<-readRDS("TutorialData/RDS/sv.table.nochim.RDS") # to load from file
```
...
```{r}
taxonomy <- as.data.frame(assignTaxonomy(sv.table.filtered, "TutorialData/rdp_train_set_14.fa.gz", multithread=TRUE, verbose=TRUE)) #assign taxonomy
```
...
#taxonomy<-readRDS("TutorialData/RDS/taxonomy.RDS") #to load from file
```

<br> One last step before we do analysis. Generate a phylogenetic tree. We will use [this](https://f1000research.com/articles/5-1492/v2) workflow to carry it out completely in R. If you were on a unix computer with QIIME installed you could also use MicrobeR's Make.Tree command. The code below would generate a new tree, however this can take some time to run so instead we will use a precomputed tree which was downloaded as part of the tutorial data. Again, if it is missing, this code will regenerate it. This method of building a tree is extremely slow, and if you were dealing with >500 features, the [Fast Tree](http://www.microbesonline.org/fasttree/) algorithm would be supperior; however, it is not native to R and installation on PCs may be difficult/impossible.

```{r, message=F}
if(file.exists("TutorialData/RDS/tree.RDS")) { #if the file exists, load from file, else build new
    tree<-readRDS("TutorialData/RDS/tree.RDS")
} else{
seqs <- taxonomy$Seq
...
weighted<-UniFrac(phyloseq(otu_table(sv.subsampled, taxa_are_rows = T), tree), weighted=T)
```

Let us look at the part of the table to get a feel for what the distance matrix looks like.

```{r}
Nice.Table(as.matrix(weighted)[1:5,1:5])
```

####2- Now do test ADONIS test.

```{r}
adonis(weighted ~ anonymized_name + Timepoint, data=metadata, permutations = 9999)
```
...
```{r}
adonis(weighted~Timepoint, 
       strata=metadata[labels(weighted),]$anonymized_name, # give the id of the person, but make sure in same order as dist matrix 
       data=metadata)
```
...
species<-assignSpecies(taxonomy[rownames(filt.results.deseq),]$Seq, "TutorialData/rdp_species_assignment_14.fa.gz", allowMultiple = TRUE, verbose=TRUE)
...
results.deseq %>%
  ...
  mutate(Significance=if_else(padj<0.1 & abs(log2FoldChange)>1, "Significant","Not Significant")) %>% #create a new category for if the result is signifcant or not
  mutate(Significance=if_else(is.na(Significance), "Not Significant", Significance)) %>% #catch NAs from when their was no FDR
  ggplot(aes(x=log2FoldChange, y=-log10(pvalue), color=Significance, label=Genus)) +
