...
In additional please manually download from this [link](https://github.com/jbisanz/BMS270_2018/raw/master/TutorialData.tar.gz), and decompress in the same directory as <i>YourName.MicrobiomeTutorial.R</i></b>
...
#error_profile<-readRDS("RDS/error_profile.RDS") #to load from file
```
...
#derep<-readRDS("RDS/derep.RDS") #to load from file
```
...
#denoised<-readRDS("RDS/denoised.RDS") #to load from file
```
...
#sv.table<-readRDS("RDS/sv.table.RDS") # to load from file
```
...
#sv.table.nochim<-readRDS("RDS/sv.table.nochim.RDS") # to load from file
```
...
#taxonomy<-readRDS("RDS/taxonomy.RDS") #to load from file
```
...
```{r, message=F}
if(file.exists("RDS/tree.RDS")) { #if the file exists, load from file, else build new
...
unweighted<-UniFrac(phyloseq(otu_table(sv.subsampled, taxa_are_rows = T), tree), weighted=F)
```
